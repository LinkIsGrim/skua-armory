...
// Client-facing rebroadcasts of the cert events above. Fired by the server's
// own handler of each canonical event via CBA_fnc_globalEvent (non-JIP), so
// hasInterface clients can react to live cert state changes without needing
// the extension to reach them directly.
